...
export function loadMidtransConfig(): MidtransConfig {
  ...
  if (!serverKey) {
    throw new Error(
      'MIDTRANS_SERVER_KEY is required in environment variables. ' +
  ...
  if (!clientKey) {
    throw new Error(
      'MIDTRANS_CLIENT_KEY is required in environment variables. ' +
      'Get your key from https://dashboard.sandbox.midtrans.com/settings/config_info'
    );
  }
  ...
  if (serverKey.includes('your-key-here') || clientKey.includes('your-key-here')) {
    console.warn(
      '⚠️  WARNING: Midtrans is using placeholder credentials.\n' +
 ...
 */
export const midtransConfig = loadMidtransConfig();
