...
let RolesGuard = class RolesGuard {
    ...
    canActivate(context) {
        ...
        if (!user) {
            throw new common_1.ForbiddenException("Akses ditolak — silakan login terlebih dahulu.");
        }
        // SUPER_ADMIN selalu punya akses penuh
        if (user.adminRole === roles_decorator_1.AdminRole.SUPER_ADMIN) {
            return true;
        }
        const hasRole = requiredRoles.some((role) => user.adminRole === role);
        if (!hasRole) {
            throw new common_1.ForbiddenException(`Akses ditolak — role ${user.adminRole || "tidak ada"} tidak memiliki izin untuk aksi ini.`);
        }
        return true;
    }
};
