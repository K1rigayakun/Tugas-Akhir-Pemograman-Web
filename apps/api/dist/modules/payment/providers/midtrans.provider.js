...
let MidtransProvider = MidtransProvider_1 = class MidtransProvider {
    ...
    async initialize(config) {
        ...
        try {
            ...
            const midtransClient = await Promise.resolve().then(() => __importStar(require('midtrans-client')));
    ...
    async createPayment(request) {
        ...
        let transaction;
        try {
            if (request.method === payment_method_enum_1.PaymentMethod.QRIS) {
            ...
            else if (request.method === payment_method_enum_1.PaymentMethod.VIRTUAL_ACCOUNT) {
                if (!request.bank) {
                    throw new common_1.BadRequestException('Bank is required for Virtual Account payment');
                }
            ...
            else if (request.method === payment_method_enum_1.PaymentMethod.EWALLET) {
                if (!request.walletType) {
                    throw new common_1.BadRequestException('Wallet type is required for E-Wallet payment');
                }
                ...
                throw new common_1.BadRequestException(`Unsupported payment method: ${request.method}`);
            }
        }
        catch (error) {
     ...
     */
    parseQRISResponse(transaction, orderId) {
        const expiresAt = new Date(transaction.expiry_time || Date.now() + 15 * 60 * 1000);
        const qrisDetails = {
            qrCodeBase64: transaction.actions?.find((a) => a.name === 'generate-qr-code')?.url || '',
            qrString: transaction.qr_string || transaction.actions?.find((a) => a.name === 'generate-qr-code')?.url || '',
     ...
     */
    parseVirtualAccountResponse(transaction, orderId, bankName) {
     ...
     */
    parseEWalletResponse(transaction, orderId, walletType) {
        const expiresAt = new Date(transaction.expiry_time || Date.now() + 15 * 60 * 1000);
        // Extract redirect URL and deep link from actions
        let redirectUrl = '';
        let deepLink = '';
        if (transaction.actions && Array.isArray(transaction.actions)) {
            const deepLinkAction = transaction.actions.find((a) => a.name === 'deeplink-redirect' || a.name === 'generate-qr-code');
            const redirectAction = transaction.actions.find((a) => a.name === 'redirect' || a.name === 'redirect-url');
...
exports.MidtransProvider = MidtransProvider = MidtransProvider_1 = __decorate([
